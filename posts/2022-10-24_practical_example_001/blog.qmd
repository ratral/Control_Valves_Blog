...
[^3]: This function will be clarified in a future blog.

The parameters corresponding to the valves are the following:[^4]

[^4]: These parameters are similar to those in reality but adjusted for this Example.
...
# Sixth step: Define criteria for valve selection:

1.  General Criteria for the inherent characteristic of valve:
    - **Flow Coefficient Range:** Operative Range it is the ratio between the normal maximum operative flow coefficient and the minimum controllable operative flow coefficient $k_{v_{min.}} \iff k_{v_{max.}}$
    ...
    - **Operation Ratio:** The Operating Ratio is the ratio of the Flow Coefficient Range to the Operation Ratio minus one. The optimal value is Zero. An Operating Ratio of Zero means that a 1% opening of the valve will increase the flow coefficient by 1%.  
...
summary_brand_valves |> 
  select(brand,	min.kv,	max.kv, kv.range,	min.pv, max.pv,	pv.range, op.ratio, min.cav,	max.cav,	mean.cav) |> 
  mutate(across(where(is.numeric), round, 2)) |> 
  gt()
